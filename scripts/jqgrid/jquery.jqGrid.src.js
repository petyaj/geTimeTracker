			...
			if(p.gridview === true) {
				...
				if(p.grouping) {
				    ...
				    try {
				        ...
				        $(self.firstElementChild).html($(self.firstElementChild).html() + rowData.join(''));
				    }
			...
			this.each(function() {
				...
				while(k < datalen) {
					...
					for(i = gi+si+ni; i < p.colModel.length;i++){
						cm = p.colModel[i];
						nm = cm.jsonmap ? cm.jsonmap : cm.name;
	...
	navGrid : function (elem, oMuligrid, pEdit,pAdd,pDel,pSearch, pView) {
		...
		return this.each(function() {
			...
			for(i = 0; i<clone; i++) {
				...
				if(p._nvtd) {
					if(twd > p._nvtd[0] && p._nvtd[0] > 0) { //p.ruc, gridProject
					...
					if(o.keys===true) {
						$(ind).bind("keydown",function(e) {
							...
							if (e.keyCode === 13) {
								debugger;
								var ta = e.target;
								if(ta.tagName === 'TEXTAREA' && !e.ctrlKey) { return true; }
